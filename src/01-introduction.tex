...
For example in \hangsheng{}\linyun{DARPA DC} dataset, 
...
which lasts for \hangsheng{Not so long}\linyun{XX} years.
Further, the MITRE ATT\&CK Framework reports \hangsheng{}\linyun{???\%} of the trojan-relevant attacks.
...
The rule-based solutions are usually commercial products \cite{milajerdi2019holmes,milajerdi2019poirot,hossain2020combating} such as SIEM (Security Information and Event Management) and \hangsheng{EDR}\linyun{YYY} detect APT attacks via predefined security rules.
...
Many researchers consider the problem of intrusion detection as a supervised-learning\hangsheng{Not all are supervised models} problem \cite{liu2018towards,hassan2019nodoze,hassan2020we} or an unsupervised-learning problem \cite{wang2022threatrace,han2020unicorn,wang2020you}.
...
we can learn a classification model to predict the attacks \hangsheng{\cite{wang2022threatrace,han2020unicorn,wang2020you}}.
...
We evaluate \tool with extensive experiments by building a \hangsheng{not only caldera}Caldera-based benchmark,
...
\begin{itemize}[leftmargin=*]
  ...
  \item We deliver our \hangsheng{not only caldera}Caldera-based benchmark, which simulates 10 trojan-induced attacks. 
