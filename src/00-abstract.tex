...
in critical sectors such as banking and government.
Trojan programs, the malware disguising normal system process,
...
e.g., graph embedding on the constructed provenance graphs.
However, given their statistical and probabilistic nature,
industrial security engineers are challenging to verify the reported alarms,
making the false alarms prohibitively expensive.

In this work, we propose \tool,
a trojan-oriented explainable intrusion detection technique,
to report disguising trojans in APT attack.
...
the behaviors of a target program (e.g., scanning the ports) and
its claimed process intention (e.g., \textit{svchost.exe}).
On one hand, we construct the normal behaviorial profile of a system process by extracting
its verifiable logical formula from LLM,
representing its behaviorial invariants.
On the other hand, we extract the runtime behavior of plausible system processes
from the system logs.
The violation of process from its normality (i.e., behaviorial invariants) serves as both the explanation and alarm.
Our extensive experiment on \linyun{XX} systems shows that 
\tool is effective in reporting trojan-based APT attack (with the precision of \linyun{100\%} and the recall of \linyun{??\%}),
